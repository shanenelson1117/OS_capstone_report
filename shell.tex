...
TODO: without the \& call join on the spawned process(es), else just let the 
process go 
\\
\\
for background tasks we might want to replicate bash behavior where it sends the process output
with like [n]: ... if you know what I mean. If we do this then we need to redirect the output of the 
background task to a file that we have prepended with this, then cat the file. We should have a call about this
...
prefer to create the file on by passing \co{FS_OPEN_ON_CORE(core) | FS_OPEN_CREATE} to \co{aos_rpc_fs_open()} as teh \co{flags} argument. This is intended to mirror
...
\begin{enumerate}
  ...
  \begin{lstlisting}
  // For the writer only (reader just sets pipe_in true and pipe_out_is_dsp 0)
