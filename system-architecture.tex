...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{images/osarch.drawio.png}
...
\section{Design goals for this project}
\label{goals}
Our main design goals are as follows:
\begin{enumerate}
      \item Allow as much parallelism and concurrency as possible. Modern systems are concurrent,
      we want our system to lean into concurrency. This has the benefits of better performance,
      but also allows the user to use our system in a concurrent matter symbiotically rather than
      fighting the underlying implementation.
      \item Given that we expect many threads to be running on our system, we want to allocate memory
      as lazily as possible. This means that memory is only allocated when it is needed. Low memory usage
      allows us to enable better concurrency.
      \item When concurrency is not effected, intelligently approach the tradeoff between performance
      and ease of implementation, code readibility, and code maintainability. Unless performance is
      significantly impacted we choose to make the code base as readable and simple as possible. This has
      helped us immensly as all milestones build on each other and frequently required editing past work.
\end{enumerate}