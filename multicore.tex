...
This milestone began with Allan implementing the main functionality in \co{coreboot_boot_core} in
\co{usr/init/coreboot.c}, where we allocated all the necessary data structures and set up the
shared URPC frame. In this part, Sahil also enabled booting all cores on the Toradex board by
modifying \co{usr/init/main.c}.

\medskip\noindent
Sahil then implemented memory allocation on remote cores, which involved allocating a large chunk of
...
\medskip\noindent
Finally, Sahil implemented all the core lifecycle functions and wrote sample test cases to demonstrate
the functionality.
\\
 \newline
Testing note: Test C22 will simply spin and timeout after printing test passed in the log. We interpret this as a misunderstanding of the test infrastrcuture on our end.