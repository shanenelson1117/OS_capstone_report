...
\chapter*{M4: Core-Local Message Passing}{Section author: Shane Nelson}
\addcontentsline{toc}{chapter}{M4: Core-Local Message Passing}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{images/m4_diag.png}
...
In order to reduce the latency for large messages, we implement a shared memory frame to pass such messages. Without shared memory
 ...
 \begin{enumerate}
    ...
    \item Client maps this frame into its virtual memory frame and updates the \newline \co{struct aos_rpc.shm_vaddr} and \co{struct aos_rpc.shm_size} fields.
...
naive mutex-based synchronization and shared memory for large messages. At this point we were passing all of the \co{m4_grading} tests.
 \newline
 \newline
Phase 2 began after a conversation on Ed with Simon which motivated Shane to implement the optimizations below. This
...
and (2) reduce the memory usage of our shared-memory approach to large LMP messages.
...
\begin{description}
    \item \textbf{1. RTT: Round Trip Trial.}
