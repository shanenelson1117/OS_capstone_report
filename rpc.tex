...
server side handlers contained in \co{usr/init/rpc_server.c}. Figure~\ref{fig:m4arch} below is a diagram illustrating the overall LMP subsystem architecture.
\begin{figure}[H]
    \centering
...
\subsubsection{Synchronous send and receive}
...
the efficacy of this optimization below, see \hyperref[sec:performance-opt]{Performance Optimization}. One area for improvement
in our implementation would be to split the services to their own processes. This would allow for better concurrency
...
future investigation. Our multithreaded stress
...
This result reflects our initial motivation for building this optimization. This may seem to be a heavily synthetic
benchmark, but we make use of this behavior in our \hyperref[sec:background-tasks]{shell}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{images/naive_purple.png}
    ...
    \includegraphics[width=0.9\textwidth]{images/perthread_purple.png}
    ...
    create a perthread channel is negligible compared to the baseline. The per-thread implementation
...
involved reworking and hardening the implementation in phase 1, as well as changing the protocol to handle
 ...
 \newline
  \newline
  Testing note: Test R02 times out rather than completeling. From discussions on ed this seems to be common behavior among all groups.
\newpage